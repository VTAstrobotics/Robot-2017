...
// with CAN forwarding. Set can id to -1 to disable can fwd.
// Disable can fwd to send commands to the master VESC.
// Last modified on 2/22/2017 by: Ryan Owens
#include <stdio.h>
#include <string.h>
#include "comm_uart.h"
...
int main(void) {
	...
	// Main loop 
	// Write to a different CAN address with each iteration
	for(;;) {
		...
		if (canTestId > 5)
			canTestId = -1;
		// Increment can ID and check serial packet to make sure can fwd works
		bldc_interface_set_forward_can(canTestId);
		}
